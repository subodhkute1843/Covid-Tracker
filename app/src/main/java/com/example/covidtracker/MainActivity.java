...
public class MainActivity extends AppCompatActivity implements AdapterView.OnItemSelectedListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mTodayRecovered = findViewById(R.id.todayrevovered);
        ...
        //how to fetch data on top of our main screen(card view)
        //api utilities
        ...
        ApiUtilities.getApiInterface().getCountryData().enqueue(new Callback<List<ModelClass>>() {
            ...
            public void onResponse(Call<List<ModelClass>> call, Response<List<ModelClass>> response) {
                ...
                //this is data for all countries which we are going to show on recyclerView
        ...
        countryCodePicker.setOnCountryChangeListener(new CountryCodePicker.OnCountryChangeListener() {
            ...
            public void onCountrySelected() {
                //it will return that country which will selected by user
                country = countryCodePicker.getSelectedCountryName();
                fetchdata();
            }
        });
    ...
    private void fetchdata() {
        ...
        ApiUtilities.getApiInterface().getCountryData().enqueue(new Callback<List<ModelClass>>() {
            @Override
            public void onResponse(Call<List<ModelClass>> call, Response<List<ModelClass>> response) {
                modelClassList.addAll(response.body()); //all country data
                //for specific country data
                for (int i=0; i<modelClassList.size(); i++){
                    if (modelClassList.get(i).getCountry().equals(country))
                    {
                        ...
                        int active, recovered, total, deaths;
                        ...
                        //updategraph(modelClassList.get(i).getCases(), modelClassList.get(i).getActive(), modelClassList.get(i).getRecovered(), modelClassList.get(i).getDeaths());

                        updategraph(active, recovered, total, deaths);
    ...
    private void updategraph(int active, int recovered, int total, int deaths) {

        mPieChart.clearChart();         //if user select other country then previous chart has to be cleared

        mPieChart.addPieSlice(new PieModel("Confirm" , total, Color.parseColor("#FFB071")));
